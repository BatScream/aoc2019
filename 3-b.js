...
function getPoints(wire, name) {
    ...
    wire.forEach((ins) => {
        if(ins.startsWith("R")){
            const right = parseInt(ins.split("R")[1]);
            for(let i=1;i<=right;i++) {
                points.push({x: x+i, y});
                numberOfSteps++;
                steps[name][(x+i)+'_'+y] = numberOfSteps;
            }
            x += right;
        }
        if(ins.startsWith("U")){
            const up = parseInt(ins.split("U")[1]);
            for(let i=1;i<=up;i++) {
                points.push({x, y: y+i});
                numberOfSteps++;
                steps[name][x+'_'+(y+i)] = numberOfSteps;
            }
            y += up;
        }         
        if(ins.startsWith("L")){
            const left = parseInt(ins.split("L")[1]);
        ...
        if(ins.startsWith("D")){
            const down = parseInt(ins.split("D")[1]);
...
function findIntersection(pointsA, pointsB){
    const intersections = [];
    pointsA.forEach((point) => {
        const result = pointsB.find(element => element.x === point.x && element.y === point.y);
        if(result){
...
const wire1Points = getPoints(wire1, 'a');
const wire2Points = getPoints(wire2, 'b');

const intersections = findIntersection(wire1Points, wire2Points);
const cumSteps = intersections.map(i => steps['a'][(i.x)+'_'+(i.y)] + steps['b'][(i.x)+'_'+(i.y)]);
console.log(cumSteps.sort((a,b) => a-b));