...
const createAmplifier = (phase, program) => {
    function amplify(signal){
        console.log(phase, program, signal);
        const codeRunner = run(program);
        console.log(codeRunner.next());
        console.log(codeRunner.next(phase));
        const output = codeRunner.next(signal);
        console.log(output);
        return output.value;
    }
    return {amplify}
...
phaseSettings.forEach(setting => {
    const amplifiers = setting.map(phase => createAmplifier(phase, [...program]));
    let signalStrength = 0;
    amplifiers.forEach(amp => {
        signalStrength = amp.amplify(signalStrength);
    });
    maxSignalStrength = (maxSignalStrength >  signalStrength) ? maxSignalStrength: signalStrength;
});

console.log(maxSignalStrength);