...
function calculate(reactions) {
    ...
    function start(chemical){
        ...
        console.log(consumedOres);
        return consumedOres;
    }
    return {start}
}

function findHighestAmtOfFuel(){
    const calculator = calculate(reactions);
    let oreRequiredForOneFuel = calculator.start(new Chemical("FUEL", 1));
    let low = 1e12/oreRequiredForOneFuel;
    let high = 10 * low;

    while (calculator.start(new Chemical("FUEL", high)) < 1e12){
        low = high
        high = 10 * low
    }
    let mid = (low + high) / 2;
    while (low < high - 1){
        mid = (low + high) / 2
        let oresRequiredForFuels = calculator.start(new Chemical("FUEL", mid));
        if (oresRequiredForFuels < 1e12){
            low = mid;
        }
        else if( oresRequiredForFuels > 1e12){
            high = mid;
        }
        else break
    }
...
console.log(findHighestAmtOfFuel());
